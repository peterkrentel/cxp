...
**Split Ollama into two instances, by model — not per-agent.** Even after the resource-limit fix (load average down from 25 to single digits), occasional timeouts are still happening on a genuinely uncontended run — root cause: every time `verifier` scores something as passing, it fans out to `assessor` (`0.5b` model) and `reflect` (`1.5b` model) *simultaneously*, every single time, regardless of test-runner concurrency. That's structural to the pipeline, not something the test runner's own sequential-submission fix touches. Splitting into two Ollama deployments — one serving the `1.5b` consumers (planner/executor/verifier/reflect), one serving just `assessor`'s `0.5b` — would put that concurrent pair on separate processes instead of competing for the same CPU allocation. Doesn't need *more* total resources than today: split the existing 5 CPU/4Gi budget into two smaller pools (~3.5 CPU/3Gi + ~1.5 CPU/1.5Gi) rather than adding a second full allocation. Real infra work (second Deployment, wiring `assessor`'s `OLLAMA_URL` separately) — not a quick tweak, but well-motivated by an actual observed pattern, not speculation.
